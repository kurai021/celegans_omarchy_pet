...
- **Minimum resolution**: 1024×768. The Expert, Lab and Circuit overlays are designed for this size. At smaller resolutions the panels scroll vertically and adapt, but the layout is optimised for 1024×768.
...
- **Paused while closed**: the pet only lives while the panel is open. Energy, sleep drive, the auto-feeder and the world all pause when you close it (`PetState.lifeActive`, `World.live`), so an overnight away never starves it — reopen and it is exactly where you left it. Only memories still fade in real time by design.
...
- **Experiment Lab (🧪)**: the "lab" button in the Mind header opens a stimulus overlay that injects through the *real* connectome — thermo onto AFD cells, chemical-B onto AWA cells, tail-touch onto PLM cells — with no bypass steering, so the response is the wiring's. A live readout shows the honest sub-threshold picture (probe → charge per channel, with a hint when you should crank a slider), fires per real cell, and motor L/R; the **⚡ ×3 amplify** toggle maps exactly to the gains the harness measured a real effect at. Tail pokes flash the body so you can *see* the stimulus land. Every experiment can be saved, re-applied (↺) or deleted (🗑) from the journal.
- **Circuit overlay (🧬)**: the "circuit" button in the Mind header opens a wiring editor for the *real* connectome. Every synapse shown is introspected from `Celegans.js` (its actual base weight — e.g. `AVL→MVL10 = −5`), and a slider drag overrides that weight absolutely ("∓" flips sign, ✕ removes the override). A filter box narrows the list to the synapses involving a typed cell (presets that don't touch it dim out). You can also add a **new** connection (base 0) between any real cell pair. The session is isolated exactly like the Experiment Lab: opening snapshots the brain *and* the active patch map, and closing restores both — "■ end & restore" rebuilds the editor from the restored wiring so the window always shows exactly what the pet runs. The only path that writes to `pet.json` is the explicit ⚡ **apply to pet** button — that patch set then survives sessions, reloads and restarts, and stays visible as a 🧬 chip next to the pet's name. Presets (chemotaxis ↑, escape kick, chemo shortcut, calm touch) are hypotheses, not guarantees: the harness in `experiments/` measures what they actually do (for 20260914 those presets do **not** rescue aux-off navigation, and they reshape rather than amplify the tail reflex — see docs).
